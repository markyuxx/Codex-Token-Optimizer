...
function writeReports(runtime, summary) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "| --- | --- | ---: | ---: | ---: | --- | --- |",
      ...summary.tasks.map((task) => `| ${task.name} | ${task.success} | ${task.tokensSaved} | ${task.recallAtK} | ${task.precisionAtK} | ${task.budgetRespected ? "yes" : "no"} | ${task.optimized.paths.join(", ")} |`),
      "",
      "## Savings By Source",
      "",
...
async function runBenchmark(runtime, options = {}) {
  ...
  const suiteTasks = options.smoke ? suite.tasks.slice(0, 5) : suite.tasks;
  ...
  for (const task of suiteTasks) {
    const taskBudget = task.budget || options.budget || 600;
    ...
    const optimizedHit = expected.length === 0 || expected.some((candidate) => optimizedPaths.some((entry) => entry === candidate || entry.endsWith(candidate)));
    ...
    const expectedSymbolsFound = expectedSymbols.filter((symbolName) => optimized.items.some((item) => item.excerpt.includes(symbolName)));
    ...
    tasks.push({
      ...
      savingsPercent: tokensBaseline ? Number((((tokensBaseline - tokensOptimized) / tokensBaseline) * 100).toFixed(2)) : 0,
      ...
      expectedFilesFound: truePositiveCount,
      expectedSymbolsFound: expectedSymbolsFound.length,
      budgetRespected: optimized.usedTokens <= optimized.requestedBudget && optimized.overBudget === false,
      staleWarningsDetected: optimized.staleWarnings.length,
      queryTimeMs,
      embeddingCost: 0,
      success: optimizedHit && !optimized.overBudget && optimized.usedTokens <= optimized.requestedBudget ? "pass" : "regression",
  ...
  const summary = {
    ...
    metrics: {
      ...
      embeddingCost: 0,
      indexTimeMs,
      indexSizeBytes: Buffer.byteLength(JSON.stringify(runtime.currentIndex.files)) + Buffer.byteLength(JSON.stringify(runtime.currentIndex.chunks)),
      ...
      cacheSavings: cacheProbe.tokensSaved,
      commandCompactionSavings: commandProbe.tokensSaved,
      budgetRespected: tasks.every((task) => task.budgetRespected),
      staleWarningsDetected: tasks.reduce((sum, task) => sum + task.staleWarningsDetected, 0),
      savingsBy: {
