...
function pathBoost(relPath) {
  ...
  if (/(^|\/)package(-lock)?\.json$/i.test(normalized)) score -= 0.8;
  return score;
}
    ...
    switch (node.type) {
      ...
      case "ClassDeclaration":
        if (node.id && node.id.name) {
          symbols.push({ name: node.id.name, kind: node.type === "ClassDeclaration" ? "class" : "function", line: node.loc?.start.line || 1 });
        }
        break;
      case "VariableDeclarator":
        if (node.id && node.id.type === "Identifier" && node.init && ["ArrowFunctionExpression", "FunctionExpression"].includes(node.init.type)) {
          symbols.push({ name: node.id.name, kind: "function", line: node.loc?.start.line || 1 });
        }
        break;
      case "CallExpression":
...
async function queryIndex(index, query, options = {}) {
  ...
  scoredChunks.forEach((chunk) => {
    chunk.score = chunk.lexicalScore + (chunk.embeddingScore || 0) + extensionBoost(chunk.ext) + pathBoost(chunk.path);
  });
